...
class BaseTool(ABC):
    ...
    async def execute(self, tool_call_params: ToolCallParams) -> Message:
        message = Message(role=Role.TOOL, name=StrictStr(tool_call_params.tool_call.function.name), tool_call_id=StrictStr(tool_call_params.tool_call.id))

        try:
            result = self._execute(tool_call_params)
            if isinstance(result, Message):
                message = result
            else:
                message.content = StrictStr(result)
        except Exception as e:
            message.content = StrictStr(e)
        return message
